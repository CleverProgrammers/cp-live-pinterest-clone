...
class ParentBasedSampler {
    ...
    shouldSample(parentContext, traceId, spanName, spanKind, attributes, links) {
        if (!parentContext) {
            return this._root.shouldSample(parentContext, traceId, spanName, spanKind, attributes, links);
        }
        if (parentContext.isRemote) {
            if (parentContext.traceFlags & api_1.TraceFlags.SAMPLED) {
                return this._remoteParentSampled.shouldSample(parentContext, traceId, spanName, spanKind, attributes, links);
            }
            return this._remoteParentNotSampled.shouldSample(parentContext, traceId, spanName, spanKind, attributes, links);
        }
        if (parentContext.traceFlags & api_1.TraceFlags.SAMPLED) {
            return this._localParentSampled.shouldSample(parentContext, traceId, spanName, spanKind, attributes, links);
        }
        return this._localParentNotSampled.shouldSample(parentContext, traceId, spanName, spanKind, attributes, links);
    }
    toString() {
