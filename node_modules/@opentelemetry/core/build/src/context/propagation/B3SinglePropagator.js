...
const b3_common_1 = require("./b3-common");
...
class B3SinglePropagator {
    inject(context, carrier, setter) {
        ...
        if (!spanContext || !api_1.isSpanContextValid(spanContext))
            return;
        const samplingState = context.getValue(b3_common_1.B3_DEBUG_FLAG_KEY) || spanContext.traceFlags & 0x1;
        const value = `${spanContext.traceId}-${spanContext.spanId}-${samplingState}`;
        setter(carrier, exports.B3_CONTEXT_HEADER, value);
    }
    extract(context, carrier, getter) {
        const header = getter(carrier, exports.B3_CONTEXT_HEADER);
        ...
        if (samplingState === DEBUG_STATE) {
            context = context.setValue(b3_common_1.B3_DEBUG_FLAG_KEY, samplingState);
        }
        return api_1.setExtractedSpanContext(context, {
            traceId,
            spanId,
            isRemote: true,
            traceFlags,
